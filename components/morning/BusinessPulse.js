...
const PULSE_CARDS = [
  {
    ...
    tone:     "danger",
    getValue: (cards) =>
      cards.filter((c) => c.status !== "Done" && c.priority === "High").length,
  },
  {
    id:       "payments",
    icon:     IndianRupee,
    ...
    getValue: (cards) => {
      const count = cards.filter(
        (c) => c.status !== "Done" && ["Invoice", "Payment"].includes(c.category)
      ).length;
      return count > 0 ? "₹48,500" : "₹0";
    },
  },
  {
    id:       "conversations",
    ...
    getValue: (cards) =>
      cards.filter(
        (c) => c.status !== "Done" && c.category === "Customer Request"
      ).length,
  },
  {
    id:       "deadlines",
...
/**
 * BusinessPulse — four flat metric cards replacing KpiCard grid.
 *
 * Props:
 *   cards  ActionCard[]
 */
  ...
  return (
    ...
    >
      {PULSE_CARDS.map((card) => {
        ...
        const value = card.getValue(cards);
